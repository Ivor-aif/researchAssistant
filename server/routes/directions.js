...
router.get('/', query('projectId').optional().isString(), async (req, res) => {
  ...
  if (projectId) {
    ...
    return res.json(rows.map(({ _id, project_id, name, description, created_at }) => ({ id: _id, project_id, name, description, created_at })))
  } else {
...
router.post(
  ...
  async (req, res) => {
    ...
    const doc = await db.directions.insert({ project_id: projectId, name, description: description || null, created_at: now, updated_at: now })
    res.json({ id: doc._id, project_id: projectId, name, description: description || null, created_at: now, updated_at: now })
  }
)
...
router.put(
  ...
  async (req, res) => {
    ...
    const now = new Date().toISOString()
    await db.directions.update({ _id: id }, { $set: { name: name ?? dir.name, description: description ?? dir.description, updated_at: now } })
    const row = await db.directions.findOne({ _id: id })
    res.json({ id: row._id, project_id: row.project_id, name: row.name, description: row.description, created_at: row.created_at, updated_at: row.updated_at })
  }
)
